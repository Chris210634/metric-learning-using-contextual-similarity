    ...
    for x, y in tqdm(trainloader):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if args.loss == 'roadmap':
                ...
                loss = (0.5 * loss_supap + 0.5 * loss_con) * (1. - args.gamma) + args.gamma * reg_loss
            elif args.loss == 'triplet':
                miner_output = miner(f, y)
                loss = triplet_criterion(f, y, miner_output) * (1. - args.gamma) + args.gamma * reg_loss
            elif args.loss == 'multisimilarity':
                loss = ms_criterion(f, y) * (1. - args.gamma) + args.gamma * reg_loss
            elif args.loss == 'smoothap':
                loss = smoothAP_criterion(scores, w) * (1. - args.gamma) + args.gamma * reg_loss
            elif args.loss == 'fastap':
                loss = fastap_criterion(f, y) * (1. - args.gamma) + args.gamma * reg_loss
            elif args.loss == 'contrastive':
                loss = margin_contrastive(scores, w, pos_margins=0.9, neg_margins=0.6) * (1. - args.gamma) + args.gamma * reg_loss
            elif args.loss == 'ntxent':
                loss = ntxent_criterion(f, y) * (1. - args.gamma) + args.gamma * reg_loss
...
if args.dataset in ['CUB', 'Cars']:
    ...
    assert False
testloader =  torch.utils.data.DataLoader(testset, batch_size=128, num_workers=16, shuffle=False, drop_last=False, pin_memory=True)
test_ks(model, testloader, ks=ks, embedding_size=args.embedding_size)